...
// === INFO PANEL ===
function InfoPanel({ node, onClose, connections, isMobile, stories, lang, t, allNodes, edges, onNodeSelect }: {
  node: any; onClose: () => void; connections: string[]; isMobile: boolean;
  stories: any[]; lang: 'en' | 'ru'; t: (k: any) => string;
  allNodes: any[]; edges: any[]; onNodeSelect: (id: string) => void;
}) {
  const color = COLORS[node.group] || "#ffffff";
  const [tab, setTab] = useState<'overview' | 'stories' | 'mindmap'>('overview');
  ...
  return (
    <div onClick={(e) => e.stopPropagation()} style={{
      ...
      maxWidth: isMobile ? (tab === 'mindmap' || tab === 'stories' ? 'calc(100vw - 20px)' : '90vw') : 340,
      maxHeight: tab === 'mindmap' || tab === 'stories' ? (isMobile ? '70vh' : 'auto') : 'auto',
      overflowY: tab === 'mindmap' || tab === 'stories' ? 'auto' : 'visible',
      fontFamily: "Inter, system-ui, sans-serif",
      backdropFilter: "blur(16px)",
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 10, borderBottom: "1px solid rgba(255,255,240,0.06)" }}>
        {(['overview', 'stories', 'mindmap'] as const).map(tabName => (
          <div key={tabName} onClick={() => setTab(tabName)} style={{
            ...
            {tabName === 'overview' ? t('overview') :
              tabName === 'stories' ? `${t('stories')} ${nodeStories.length}` :
              lang === 'ru' ? '🗺 карта связей' : '🗺 mind map'}
          </div>
        ))}
      </div>
      ...
      {tab === 'stories' && <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, overflow: "visible" }}>
          {nodeStories.slice(0, 15).map((s: any, i: number) => (
            <StoryCard key={i} story={s} color={color} isMobile={isMobile} lang={lang} />
          ))}
...
function EntryNode({ data, color, onSelect, isSelected, isMobile, highlight, hidden }: {
  ...
  useFrame(({ clock }) => {
    ...
    if (meshRef.current.material) {
      ...
      const baseIntensity = isSelected ? 2.5 : 1.0;
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      <mesh ref={meshRef} onClick={(e) => { e.stopPropagation(); onSelect(); }}>
        ...
        <meshStandardMaterial color={color} emissive={color} emissiveIntensity={isSelected ? 2.5 : 1.0} />
      </mesh>
...
function CameraController({ isMobile, autoRotate, selectedNode }: { isMobile: boolean; autoRotate: boolean; selectedNode: any }) {
  ...
  useFrame(() => {
    if (selectedNode && ref.current) {
      targetPos.set(selectedNode.x, selectedNode.y, selectedNode.z);
  ...
  return (
    <div style={{ width: "100%", height: "100%", position: "relative", overflow: "hidden" }}>
      ...
      >
        ...
        <fogExp2 attach="fog" args={["#07070d", 0.006]} />
        <Stars />
        <FloatingSparkles />
      ...
      {selectedNode && (
        <div style={{
          ...
          pointerEvents: "auto", zIndex: 999999,
        }}>
          <InfoPanel
            node={selectedNode}
