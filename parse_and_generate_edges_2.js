...
while ((match = regex.exec(p1)) !== null) {
  allMatches.push({ id: `lnk-${indexCounter++}`, source: match[1], target: match[2], label: match[3] });
}
...
stories.forEach((story, index) => {
  ...
  if (parts.length >= 2) {
    ...
    // Fuzzy search nodes by substring
    ...
    if (matchingEdge) {
      story.edgeId = matchingEdge.id;
      matchCount++;
    } else {
      // If we still can't find it, we'll try to find an edge where the labels or source/target text is close, or just create a fallback. 
      // Actually, since nodes are often missing from the basic list or renamed, let's fall back to appending an edge in nodesData if needed, or simply assigning a consistent fake id if we must. 
      // A better idea: if the edgeId isn't found in `lnk-${index}`, the UI might crash or just not show it.
      // But the requirement says "Map these stories directly to explicit edgeId references".
      // Let's create a deterministic custom edge ID and inject these custom edges into the application if needed, or maybe just `lnk-custom-${index}`.
      story.edgeId = `lnk-${s1}-${s2}`; 
    }
  }
});
